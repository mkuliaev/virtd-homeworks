...
Проблема заключается в том, что Nginx теперь слушает на порту 81 внутри контейнера, но Docker по-прежнему пробрасывает 
порт 8080 на хосте к порту 80 внутри контейнера. Так как Nginx больше не слушает порт 80, запросы на порт 8080 хоста не проходят.
```
